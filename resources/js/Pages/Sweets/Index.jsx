...
export default function Index({ auth, sweets }) {
    ...
    const deleteSweet = (id) => {
        if (confirm('Are you sure you want to delete this sweet?')) {
            destroy(route('sweets.destroy', id));
        }
    };
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900">
                            <Link href={route('sweets.create')} className="btn-primary mb-4">Add New Sweet</Link>
                                    ...
                                    {sweets.data.map((sweet) => (
                                        <tr key={sweet.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <Link href={route('sweets.edit', sweet.id)} className="text-indigo-600 hover:text-indigo-900 mr-2">Edit</Link>
